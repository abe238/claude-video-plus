...
- **The benchmark is honest but small.** It is one deep-dive with LLM judges and blind paired panels, plus a 4-video/12-question development battery ([multi-video battery](docs/benchmarks/2026-07-12-multi-video-battery/)): overall quality parity at a large token reduction, wins concentrated on videos 8 minutes and up, and all 3 questions lost on the one short video — so prefer the standard modes for short videos. These are measurements with raw data you can audit, not a preregistered statistical trial. The confirmatory protocol is specified in the [v1 master plan](docs/plans/V1.0-MASTER-PLAN.md) and [measurement contract](docs/execution/v1/MEASUREMENT.md) and runs before any stronger claim.
